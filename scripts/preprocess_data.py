...
class DataPreprocessor:
    ...
    def fit_preprocessing(self, train_df, column_types):
        ...
        if column_types['numeric']:
            print(f"Fitting StandardScaler for {len(column_types['numeric'])} numeric columns")
        ...
        if column_types['categorical']:
            print(f"Fitting OneHotEncoder for {len(column_types['categorical'])} categorical columns")
    ...
    def transform_data(self, df):
        ...
        if 'numeric_scaler' in self.transformers and column_types['numeric']:
            ...
            numeric_data_imputed = self.transformers['numeric_imputer'].transform(numeric_data)
            numeric_data_scaled = self.transformers['numeric_scaler'].transform(numeric_data_imputed)
            ...
            result_df = pd.concat([result_df.drop(columns=column_types['numeric']), numeric_df], axis=1)
        ...
        if 'categorical_encoder' in self.transformers and column_types['categorical']:
            ...
            categorical_data_imputed = self.transformers['categorical_imputer'].transform(categorical_data)
            categorical_data_encoded = self.transformers['categorical_encoder'].transform(categorical_data_imputed)
            
            cat_feature_names = self.transformers['categorical_encoder'].get_feature_names_out(column_types['categorical'])
            categorical_df = pd.DataFrame(
                categorical_data_encoded,
                columns=cat_feature_names,
                index=df.index
            )
            result_df = pd.concat([result_df.drop(columns=column_types['categorical']), categorical_df], axis=1)
    ...
    def save_processed_data(self, train_df, val_df, test_df, run_id, output_dir):
        ...
        for split_name, split_df in [('train', train_df), ('val', val_df), ('test', test_df)]:
            ...
            print(f"✓ Saved {split_name}: {parquet_file.name} ({len(split_df)} rows, {len(split_df.columns)} cols)")
    ...
    def process(self, data_file, output_dir, transformers_dir, configs_dir):
        ...
        self.save_processed_data(train_df_transformed, val_df_transformed, test_df_transformed, run_id, output_dir)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  # Process NHANES data (suffix auto-extracted from filename)
  python preprocess_data.py \\
    --data data/nhanes/2017-2020/combined/nhanes_2017_2020_combined.parquet
  # Output: data/processed/nhanes_2017_2020_combined/
  
  # Process diabetes data
  python preprocess_data.py \\
    --data data/diabetes_data/diabetic_data.csv \\
    --seed 42
  # Output: data/processed/diabetic_data/
    ...
    result = preprocessor.process(data_file, output_dir, transformers_dir, configs_dir)
